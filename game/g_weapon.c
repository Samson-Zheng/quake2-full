...
void fire_blaster (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	...
	boltLeft->s.modelindex = 0;
	boltLeft->s.sound = 0;
	boltLeft->owner = self;
	boltLeft->touch = blaster_touch;
	boltLeft->nextthink = level.time + 1;
	...
	boltRight->s.modelindex = 0;
	boltRight->s.sound = 0;
	boltRight->owner = self;
	boltRight->touch = blaster_touch;
	boltRight->nextthink = level.time + 1;
	...
	boltUp->s.modelindex = 0;
	boltUp->s.sound = 0;
	boltUp->owner = self;
	boltUp->touch = blaster_touch;
	boltUp->nextthink = level.time + 1;
	...
	boltDown->s.modelindex = 0;
	boltDown->s.sound = 0;
	boltDown->owner = self;
	boltDown->touch = blaster_touch;
	boltDown->nextthink = level.time + 1;
...
void fire_grenade (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
{
	...
	grenade->s.modelindex = gi.modelindex ("models/objects/grenade/g_dballs.md2");
	grenade->owner = self;
...
void fire_grenade2 (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
	...
	VectorClear (grenade->mins);
	VectorClear (grenade->maxs);
	grenade->s.modelindex = gi.modelindex ("models/objects/grenade2/g_dballs.md2");
	grenade->owner = self;
	grenade->touch = Grenade_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = Grenade_Explode;
